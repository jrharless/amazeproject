...
public class FirstPersonDrawer extends DefaultViewer {
	...
	public void redraw(MazePanel panel, StateGUI state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		//panel.setGC2(panel.castgraphics(panel.getGC())) ;

		// became necessary when lines of polygons that were not horizontal or vertical
		// looked ragged
//		panel.getGC2().setRenderingHint(panel.getHints().KEY_ANTIALIASING,
//				panel.getHints().VALUE_ANTIALIAS_ON);
//		panel.getGC2().setRenderingHint(panel.getHints().KEY_INTERPOLATION,
//				panel.getHints().VALUE_INTERPOLATION_BILINEAR);
//
		...
		//
		drawAllVisibleSectors(bsp_root, panel);
	}
	...
	private void drawBackground(MazePanel panel) {
		Rect bot =  new Rect(0, 0, view_width, view_height/2);
		Paint black = new Paint();
		black.setColor(Color.BLACK);
		Globals.panel.getGraphics().drawRect(bot, black);

		//panel.getGraphics().setColor(Color.DKGRAY);
		//panel.getGraphics().fillRect(0, view_height/2, view_width, view_height/2);
		Rect top = new Rect(0, view_height/2, view_width, view_height);
		Paint gray = new Paint();
		gray.setColor(Color.GRAY);
		panel.getGraphics().drawRect(top, gray);
	}
	 ...
	 */
	private void drawAllVisibleSectors(BSPNode nn, MazePanel panel) {
		traverse_node_ct++; // debug

		// Anchor, stop recursion at leaf nodes
		if (nn.isIsleaf()) {
			drawAllSegmentsOfASector((BSPLeaf) nn, panel);
			return;
		}
		...
		if ((dot >= 0) && (boundingBoxIsVisible(rch))) {
			drawAllVisibleSectors(rch, panel);
		}
		// consider left node
		if (boundingBoxIsVisible(lch))
			drawAllVisibleSectors(lch, panel);
	 ...
	 */
	private void drawAllSegmentsOfASector(BSPLeaf n, MazePanel panel) {
	 ...
	 */
	private void drawSegment(Seg seg, MazePanel panel) {
...
//			poly.lineTo(xps[1], yps[1]);
//			poly.lineTo(xps[2], yps[2]);
//			poly.lineTo(xps[3], yps[3]);
//			poly.lineTo(xps[0], yps[0]);
//			poly.close();
//			panel.getColor().setStyle(Paint.Style.FILL);
//			Paint paint2 = new Paint();
//			paint2.setColor(Color.WHITE);
//			paint2.setStyle(Paint.Style.FILL);
//			//panel.getGraphics().fillPolygon(xps, yps, 4);
//			panel.getGraphics().drawPath(poly,paint2);
			Globals.panel.setColor(p2);
