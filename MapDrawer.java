...
public class MapDrawer extends DefaultViewer {
	...
	public void draw_map(MazePanel panel, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		...
		paintwhite.setColor(Color.RED);


		panel.setColor(paintwhite);
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				paintgray.setStrokeWidth(3);
				paintgray.setColor(Color.WHITE);

				panel.setColor(seencells.hasWall(x,y, CardinalDirection.North) ? paintwhite : paintgray);
				if ((seencells.hasWall(x,y, CardinalDirection.North) || showMaze) && theCondition)
					panel.getGraphics().drawLine(nx1, ny1, nx2, ny1,panel.getColor());
				...
				panel.setColor(seencells.hasWall(x,y, CardinalDirection.West) ? paintwhite : paintgray);
				if ((seencells.hasWall(x,y, CardinalDirection.West) || showMaze) && theCondition)
					panel.getGraphics().drawLine(nx1, ny1, nx1, ny2,panel.getColor());
	...
	public void draw_currentlocation(MazePanel panel, int view_dx, int view_dy) {
		...
		// now the drawing
		panel.getGraphics().drawLine(ctrx, ctry, aptx, apty,panel.getColor());
		panel.getGraphics().drawLine(aptx, apty, aptx2 + ptofx, apty2 + ptofy,panel.getColor());
		panel.getGraphics().drawLine(aptx, apty, aptx2 - ptofx, apty2 - ptofy,panel.getColor());
	}
	...
	public void draw_solution(MazePanel panel, int offx, int offy, int px, int py) {
		...
		while (distance > 1) {
			...
			panel.getGraphics().drawLine(nx1, ny1, nx2, ny2,paintyellow);
